    ...
    def __getitem__(self, index):
        ...
        if self.mode == "val":
            if user in self.user2train:
                seq = self.user2train[user] + self.user2val[user]
            else:
                seq = self.user2val[user]
            label = self.user2val[user]
        else:
            if user in self.user2train and user in self.user2val:
                seq = self.user2train[user] + self.user2val[user]
            elif user in self.user2train:
                seq = self.user2train[user]
            elif user in self.user2val:
                seq = self.user2val[user]
            else:
                seq = []
            # seq = self.user2train[user] + self.user2val[user]
