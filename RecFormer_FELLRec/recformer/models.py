...
class RecformerForSeqRec_client(RecformerForSeqRec):
    def __init__(self, config: RecformerConfig, original_model, k):
        super().__init__(config)
        self.longformer = deepcopy(original_model.longformer)
        
        # keep the first k layers and the last layer
        self.longformer.encoder.layer = nn.ModuleList(
            [deepcopy(self.longformer.encoder.layer[i]) for i in range(k)] + [deepcopy(self.longformer.encoder.layer[-1])]
        )
        
        self.sim = deepcopy(original_model.sim)

class RecformerForSeqRec_server(RecformerForSeqRec):
    def __init__(self, config: RecformerConfig, original_model, k):
        super().__init__(config)
        self.longformer = deepcopy(original_model.longformer)
        n = len(self.longformer.encoder.layer)
        # keep the [k+1,-1] layer
        self.longformer.encoder.layer = nn.ModuleList(
            [deepcopy(self.longformer.encoder.layer[i]) for i in range(k, n-1)]
        )
        self.sim = deepcopy(original_model.sim)